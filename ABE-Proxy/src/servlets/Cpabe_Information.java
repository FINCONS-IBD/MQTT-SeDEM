...
public class Cpabe_Information extends HttpServlet {
	...
	private Logger logger = Logger.getLogger(Cpabe_Information.class);
...
//	        long startTimeAES = System.currentTimeMillis();
	        ...
	        if(shared_sym_key == null)
	        	logger.info("The value of shared sym key is null!!");
	        else
	        	logger.info("The value of shared sym key is valid!!");
...
//			calendar.add(Calendar.SECOND, 5); //Key expiration date
			calendar.add(Calendar.SECOND, this.expirationTime);
...
//    				else if(sp.getName().equals("db_auth_pwd")) db_auth_pwd = sp.getValue();
    			...
    			logger.info("Saving the key in the key storage: " + urlPost);
